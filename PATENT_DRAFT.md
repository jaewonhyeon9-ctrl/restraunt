...
> 작성일: 2026-04-29 / **갱신: 2026-05-06** (청구 후보 11→13개)
> 용도: 변리사 미팅 / 특허 출원 명세서 작성 기초 자료
> 형식: 텍스트 + Mermaid 도면 (변리사가 워드/한글로 옮기기 용이)
>
> **2026-05-06 변경 요약**:
> - 청구 후보 10 (출퇴근 GPS) — 정확도 인지 검증, 다중측정, 사장 우회 푸시로 강화
> - 청구 후보 12 신설 — 사진 인증 퇴근 필수 체크리스트
> - 청구 후보 13 신설 — 거래처/제조사/10g당가격 정규화 비교
...
| 12 | **사진 인증 퇴근 필수 체크리스트** | 사장이 항목별로 "사진 인증 필수"·"퇴근 시 필수" 두 플래그 독립 설정. 직원이 사진 미첨부 시 서버측 체크 거부, 클럭아웃 시점에 미완료 필수 항목 자동 차단 |
| 13 | **거래처·제조사·10g당가격 정규화 비교** | 이질 단위·포장(예: 5kg 봉지 / 350g 팩) 식자재의 단가를 패키지 중량(g)으로 정규화하여 10g당 가격으로 환산, 거래처·제조사 간 동일 기준 비교 가능 |
...
### 청구 후보 10: GPS 정확도 인지 출퇴근 검증 + 다중측정 + 사장 우회 요청을 결합한 일급 자동 산출 방법

> **2026-05-06 강화**: 종래의 단순 "거리 ≤ 반경" 검증은 실내·도심 빌딩의 GPS 오차(±50~150m)로 무용지물이었으나, 본 발명은 ① 정확도 보정 ② 다중측정 ③ 사장 푸시 우회 3중 결합으로 실사용 가능성을 확보.

**기술적 사상**: 직원의 휴대폰 PWA에서 클럭인/클럭아웃 시 **GPS 좌표 + 정확도 + 사업장 등록 좌표**를 비교하여 **정확도를 반영한 동적 허용 거리**(`반경 + min(정확도, 200m)`) 내에서만 인정. 단일 위치 측정의 오차를 줄이기 위해 **3초간 다중 위치 fix를 수집**하여 가장 정확한 값을 자동 선택하며, ±10m 이하 양호 정확도 도달 시 즉시 종료. GPS로 거부된 직원은 1탭으로 사장에게 푸시 알림을 보낼 수 있고, 사장은 매장 탭에서 GPS 검증 우회 토글로 즉시 해결. 클럭아웃 시점에 시급/월급 기반 일급을 즉시 환산하여 일일 손익 리포트에 자동 반영.
...
**핵심 청구 포인트**:
- **GPS 정확도 인지 동적 검증** — `허용 거리 = 반경 + min(정확도, 200m)`. 정확도 ±10m면 반경 그대로, ±100m면 100m 보정. 200m 캡으로 무한대 우회 방지.
- **다중측정 자동 선택** — `watchPosition`으로 3초간 N개 fix 수집 → 최소 정확도 자동 선택. 양호 정확도(≤10m) 즉시 종료로 UX 단축.
- **사장 측 우회 요청 1탭 푸시** — 직원의 GPS 거부 화면에서 1탭 → 매장의 모든 OWNER/MANAGER에게 푸시 (직원명, 거리, 정확도, 매장 탭 deep-link 포함).
- **사장 위치 등록 시 정확도 가드** — 사장 측 "현재 위치로 설정" 버튼이 ±100m 초과 정확도면 저장 거부, ±30~100m이면 경고. 매장 기준점이 모든 직원 출퇴근의 기준이라 부정확하면 전체가 깨짐을 방지.
...
### 청구 후보 12: 사진 인증 퇴근 필수 체크리스트 방법

**기술적 사상**: 식당 운영 중 마감·청소 등 **퇴근 전 반드시 수행해야 하는 항목을 단순 체크박스가 아닌 사진 증빙으로 강제**하고, 사진 미첨부 시 서버측에서 체크를 거부하며, 클럭아웃 시점에 미완료 필수 항목이 있으면 **클럭아웃 자체를 차단**하는 방법. 사장은 항목별로 두 플래그(`requiresPhoto`, `requiredOnClockOut`)를 독립 설정하여 항목의 중요도와 증빙 필요성을 분리 제어.
...
sequenceDiagram
  ...
  actor E as 직원
  participant W as PWA
  participant API as 서버 /api/checklist
  participant U as 업로드 /api/upload/checklist-photo
  participant CDN as Cloudinary
  participant D as DB

  S->>API: 템플릿 PATCH<br/>{requiresPhoto:true, requiredOnClockOut:true}
  E->>W: 항목 탭 (아직 미체크)
  W->>W: requiresPhoto 검사 → 사진 없음 감지
  W->>E: 카메라/갤러리 픽커 자동 호출
  E->>W: 사진 선택
  W->>U: multipart 이미지
  U->>U: 검증 (image/*, ≤8MB)
  U->>CDN: 압축 + 업로드 (자동 quality:auto:good, 1600px 제한)
  CDN-->>U: 안전한 URL
  U-->>W: photoUrl
  W->>API: POST {templateId, isChecked:true, photoUrl}
  API->>API: 사진 필수 + photoUrl 없으면 거부
  API->>D: ChecklistLog upsert (photoUrl 포함)
  API-->>W: OK
  
  Note over E,W: ── 이후 퇴근 시점 ──
  E->>W: 클럭아웃 버튼 탭
  W->>API: GET /api/checklist
  API-->>W: 항목 목록 + isChecked + requiredOnClockOut
  W->>W: requiredOnClockOut && !isChecked 필터
  alt 미완료 필수 항목 있음
    W->>E: 차단 + 미완료 N개 안내
  else 모두 완료
    W->>API: POST /api/attendance {type:'out'}
  end
```

**핵심 청구 포인트**:
- **이중 플래그 독립 제어** — `requiresPhoto`(사진 증빙 필요)와 `requiredOnClockOut`(퇴근 시 필수) 분리. "사진 X + 퇴근 필수", "사진 O + 퇴근 비필수" 등 4가지 조합 표현 가능.
- **서버측 강제** — 클라이언트에서 사진 없이 체크 요청해도 서버가 거부 (UI 우회 방지).
- **CDN 자동 압축 + 1600px 제한** — 직원이 큰 사진을 찍어도 자동 최적화로 저장 비용 절감.
- **클럭아웃 흐름 차단** — 단순 경고가 아닌 **실제 클럭아웃 API 호출 직전 검사**로 마감 누락 0건 보장.
- **유사 솔루션 차별화** — 종래 체크리스트 SaaS는 단순 체크박스 + 메모만 제공. 본 발명은 증빙 강제 + 흐름 차단 결합으로 차별.

**부가 효과**:
- 사장이 매장에 부재 중에도 마감 품질을 사후 사진 확인으로 검증 가능
- 분쟁 시 (위생/안전 이슈 등) 일자별 마감 사진을 증거 자료로 사용 가능

---

### 청구 후보 13: 거래처·제조사·10g당가격 정규화 비교 방법

**기술적 사상**: 식당 식자재는 거래처·제조사·포장 단위가 모두 달라(예: A거래처 5kg 봉지 50,000원 vs B거래처 350g 팩 1,500원) 단순 단가 비교가 불가능한데, 각 품목에 **패키지 1개당 중량(g)**을 부가 정보로 등록받아 `10g당 가격 = 단가 / 패키지중량 × 10`의 정규화된 단위가격을 자동 산출하고, 이를 정렬 키로 사용해 거래처·제조사 간 동일 기준 비교를 가능하게 하는 방법.

**도 15. 단가 정규화 비교 흐름**
```mermaid
flowchart TB
  Raw[원본 데이터<br/>거래처A: 돼지고기 5kg 50000원<br/>거래처B: 돼지고기 1kg 12000원<br/>거래처C: 돼지고기 350g 4500원] --> Norm[정규화 엔진]
  Norm --> Calc[10g당 가격 환산<br/>A: 100원 / B: 120원 / C: 128.6원]
  Calc --> Group[거래처별 그룹화]
  Group --> Sort[10g당 가격 정렬]
  Sort --> UI[비교 UI]
  UI --> Decision[가장 싼 거래처 즉시 식별]
  
  Calc --> Cost[메뉴 레시피 원가율 계산<br/>레시피 qty(g) × 10g당가격 / 10]
  Cost --> Profit[메뉴별 수익성 분석]
```

**핵심 청구 포인트**:
- **이질 단위·포장 정규화** — kg / g / 개 등 혼재 단위에서도 동일한 10g당 단위로 환산.
- **자동 계산 + 단가 누락 표시** — `unitPrice`만 입력된 품목은 패키지 중량 미입력 시 정규화 가격을 표시하지 않고 사용자에게 입력 유도.
- **거래처별 그룹 + 정렬 결합** — 같은 품목의 여러 거래처 가격을 한눈에 비교 (검색·정렬 통합).
- **레시피 원가율 정확도 향상** — 정규화된 10g당 가격이 메뉴 레시피의 사용량(g)과 곱해져 더 정확한 원가 계산 가능.
- **엑셀 일괄 등록 호환** — 엑셀 컬럼 자동 매핑 (`제조사`/`패키지중량`/`중량g` 등 별칭 인식)으로 대량 등록도 지원.
...
### 청구항 7 (기록 매체항)
청구항 1, 3, 6, 8, 9, 10, 11, 12 중 어느 한 방법을 컴퓨터에 실행시키기 위한 프로그램이 기록된, 컴퓨터로 읽을 수 있는 기록 매체.
...
### 청구항 9 (방법항 — GPS 정확도 인지 출퇴근 검증 + 다중측정 + 우회 푸시 결합 일급 산출)
**식당 직원의 출퇴근 데이터로부터 일급을 자동 산출하는 방법**으로서,
(a) 직원 단말로부터 출근 요청을 수신할 때 GPS 좌표 및 그 정확도(accuracy) 값을 수신하는 단계;
(b) 사업장에 등록된 좌표 및 반경과 비교하여 **허용 거리 = 반경 + min(정확도, 사전 설정된 상한값)**의 동적 허용 거리 내일 때만 출근으로 인정하는 단계;
(c) 상기 (a) 단계의 GPS 좌표 수신은, 직원 단말이 사전 설정된 시간 동안 복수의 위치 fix를 수집하고 그 중 가장 정확도가 높은 fix를 선택하여 전송하되, 정확도가 사전 설정된 임계값 이하에 도달하면 즉시 종료하는 다중측정 절차를 포함하는 단계;
(d) (b) 단계에서 거부된 경우, 직원 단말의 1회 사용자 입력에 의해 해당 매장의 사장(OWNER) 또는 점장(MANAGER) 단말로 직원명·거리·정확도·매장 화면 deep-link을 포함한 푸시 알림이 발송되는 단계;
(e) 사업장 좌표 등록 시 사장 단말의 정확도가 사전 설정된 상한값을 초과하는 경우 저장을 거부하는 단계;
(f) 퇴근 요청을 동일하게 GPS 검증한 후, 출근 시각과의 차이를 분 단위 근무 시간으로 산출하는 단계;
(g) 직원에 등록된 시급 또는 월급 정보를 조회하여 시급제는 (근무분/60)×시급, 월급제는 월급×(근무분/월기준근무분)으로 일급을 환산하는 단계;
(h) 환산된 일급을 일일 손익 리포트의 인건비 항목에 자동 누적하는 단계;
를 포함하는 방법.
...
### 청구항 11 (방법항 — 사진 인증 퇴근 필수 체크리스트)
**식당 운영자가 등록한 체크리스트 항목의 수행을 사진 인증으로 강제하고 퇴근 시 미완료를 차단하는 방법**으로서,
(a) 운영자 단말로부터 체크리스트 항목별로 사진 인증 필수 여부(requiresPhoto)와 퇴근 시 필수 여부(requiredOnClockOut)의 두 플래그를 독립적으로 수신·저장하는 단계;
(b) 직원 단말에서 사진 필수 항목을 체크하려 할 때, 카메라 또는 갤러리 픽커를 자동 호출하는 단계;
(c) 선택된 사진을 멀티파트 업로드하여 자동 압축 + 최대 가로 픽셀 제한을 적용한 후 CDN URL을 발급받는 단계;
(d) 체크 요청을 서버가 수신할 때, requiresPhoto가 true인데 photoUrl이 없으면 서버측에서 체크를 거부하는 단계;
(e) 직원이 클럭아웃 요청을 보낼 때, 서버 또는 클라이언트가 requiredOnClockOut이 true이면서 isChecked가 false인 항목 존재 여부를 검사하여 해당 항목이 있으면 클럭아웃을 차단하고 미완료 항목을 안내하는 단계;
를 포함하는 방법.

### 청구항 12 (방법항 — 거래처·제조사·10g당가격 정규화 비교)
**이질 단위·포장의 식자재 단가를 정규화하여 거래처 간 비교를 가능하게 하는 방법**으로서,
(a) 재고 품목 등록 시 단가(unitPrice)와 별도로 패키지 1개당 중량(packageWeightG)을 부가적으로 입력받는 단계;
(b) 등록된 품목 각각에 대해 정규화된 단위가격을 `정규화 단가 = 단가 / 패키지 중량 × N` (N은 비교 단위, 예: 10g) 식으로 자동 산출하는 단계;
(c) 거래처(supplier) 또는 제조사(manufacturer)를 그룹핑 키로 사용하여 동일 품목의 여러 공급원을 동일 화면에 표시하고, 정규화 단가를 정렬 키로 사용해 가장 저렴한 공급원을 식별 가능하게 하는 단계;
(d) 패키지 중량이 미입력된 품목은 정규화 단가를 표시하지 않고 사용자에게 입력을 유도하는 단계;
(e) 산출된 정규화 단가를 메뉴 레시피의 사용량(g)과 곱해 메뉴별 원가율 계산에 활용하는 단계;
를 포함하는 방법.
